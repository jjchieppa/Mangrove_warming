...
# tiff(file = "V_Mangrove in situ R by air temperature (four panel).tiff", height = 8, width = 12, res = 600, units = "in", compression = "zip+p")
...
K<-273.15; df$alpha<-df$Temp_C.mean+K; df$beta<-K+25
q10 <- function(x) {1.24803 * (1.74862) ^ (((x+273.15)-298.15)/10)}
points(q10(seq(20,30,1)) ~ seq(20,30,1), type = "l", lwd = 2, col = "grey50")
...
K<-273.15; df$alpha<-df$Temp_C.mean+K; df$beta<-K+25
q10 <- function(x) {13.6756 * (1.4385) ^ (((x+273.15)-298.15)/10)}
points(q10(seq(20,30,1)) ~ seq(20,30,1), type = "l", lwd = 2, col = "grey50")
...
K<-273.15; df$alpha<-df$Temp_C.mean+K; df$beta<-K+25
q10 <- function(x) {8.8903 * (1.373004) ^ (((x+273.15)-298.15)/10)}
points(q10(seq(20,30,1)) ~ seq(20,30,1), type = "l", lwd = 2, col = "grey50")
